...
export default function CreateLoanModal({ isOpen, onClose, onSuccess }: CreateLoanModalProps) {
  ...
  const onSubmit = async (data: CreateLoanFormValues) => {
    ...
    try {
      // Use the connected wallet address from the wallet context
      // If not connected, we'll display an error message to the user
      if (!walletAddress) {
        throw new Error("Wallet is not connected. Please connect your wallet to create a loan.");
      }
